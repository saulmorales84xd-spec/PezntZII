...
import net.saullmc.pezntz.client.item.JeringaRenderer;
...
public abstract class JeringaItem extends Item implements GeoItem {

    public static final String CONTROLLER = "jeringa_controller";
    ...
    public static final double ALCANCE = 3.5D;

    private static final String TAG_OBJETIVO = "JeringaObjetivo";

    private static volatile Item enUso = null;

    public static void setEnUso(Item item) {
        enUso = item;
    }

    public static boolean estaEnUso(Item item) {
        return enUso == item;
    }
    ...
    protected JeringaItem(Properties properties, String nombre) {
        super(properties);
        this.nombre = nombre;
    }
    ...
    protected abstract void aplicar(ServerPlayer usuario, Player receptor);

    @Override
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, CONTROLLER, 4,
                state -> state.setAndContinue(estaEnUso(this) ? USE : IDLE)));
    }
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        consumer.accept(new IClientItemExtensions() {
            private JeringaRenderer renderer;

            @Override
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                if (this.renderer == null) {
                    this.renderer = new JeringaRenderer(JeringaItem.this.nombre);
                }
    ...
    public ItemStack finishUsingItem(ItemStack pStack, Level pLevel, LivingEntity pLivingEntity) {
        if (!pLevel.isClientSide && pLivingEntity instanceof ServerPlayer usuario) {
            ...
            aplicar(usuario, recuperarObjetivo(pStack, usuario));
    ...
    public static Player buscarObjetivo(Player usuario) {
